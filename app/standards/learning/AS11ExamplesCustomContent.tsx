...
import {
  ...
  judicialCases,
  examCorner,
} from './AS11ExamplesData'

interface AS11ExamplesCustomContentProps {
...
export function AS11ExamplesCustomContent({
  navigateToPdfPage,
  renderTextWithReferences,
}: AS11ExamplesCustomContentProps) {
    ...
    <div className="w-full space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-slate-50 dark:bg-[#181E30] border border-slate-200 dark:border-slate-800 rounded-xl p-6 sm:p-8 space-y-4">
        <h4 className="font-sans font-bold text-xs uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
          <Info size={14} />
          <span>AS 11 Foreign Exchange Rate Translation Guide</span>
        </h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px] leading-relaxed">
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-blue-600 dark:text-blue-400 block mb-2">Monetary Items translation rule:</span>
            <p className="text-slate-650 dark:text-gray-400 text-[12px] leading-relaxed">
              Monetary items (like cash, receivables, payables) must be translated at the **closing rate** of the balance sheet date. Any exchange difference is taken to the Statement of Profit and Loss.
            </p>
          </div>
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-amber-600 dark:text-amber-400 block mb-2">Non-Monetary Items translation rule:</span>
            <p className="text-slate-650 dark:text-gray-400 text-[12px] leading-relaxed">
              Non-monetary items (like property, plant, equipment, inventory, equity shares) carried at cost must remain translated at the **transaction date rate** and are not revalued.
            </p>
          </div>
        </div>
      </div>
