...
import {
  ...
  judicialCases,
  examCorner,
} from './AS13ExamplesData'

interface AS13ExamplesCustomContentProps {
...
function CaseStudyCard({
  ...
  const themeMap = {
    ...
    emerald: {
      ...
      number: 'bg-emerald-100 text-emerald-750 dark:bg-emerald-900/40 dark:text-emerald-300 border-emerald-200/40 dark:border-emerald-800/40',
  ...
  return (
    <div id={`item-${id}`} className={`w-full bg-white dark:bg-[#111726] border border-slate-200 dark:border-gray-800 rounded-xl p-5 sm:p-7 shadow-xs border-l-4 ${c.leftBorder} flex flex-col gap-5`}>
        ...
        <div className="flex items-center gap-2.5">
          ...
          {pdfPage && navigateToPdfPage && (
            <button
              ...
              className="inline-flex items-center gap-1 text-[10px] font-bold text-red-655 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 bg-red-50 hover:bg-red-100/70 dark:bg-red-950/30 px-2 py-0.5 border border-red-200/40 dark:border-red-900/30 rounded cursor-pointer transition-all"
            >
              <FileText size={10} />
      ...
      <div className="flex flex-row items-center gap-1 border-b border-slate-155 dark:border-gray-800 overflow-x-auto scrollbar-none pb-0.5">
        {panels.map((p, idx) => (
          <button
            key={idx}
      ...
      <div className="flex-1 text-[14px] sm:text-[14.5px] text-slate-700 dark:text-gray-350 leading-relaxed font-normal min-h-[120px] transition-all duration-150">
        {panels[activePanelIdx]?.content}
      </div>

      {/* Exam Focus Box */}
      {examFocus && (
        <div className="p-4 rounded-xl border border-rose-100 dark:border-red-950/40 bg-rose-50/20 dark:bg-[#2A1D1D]/30 space-y-2 mt-2">
          ...
          <p className="text-[13px] text-slate-650 dark:text-gray-405 leading-relaxed font-normal">
            {examFocus}
          </p>
        </div>
      )}
...
export function AS13ExamplesCustomContent({
  navigateToPdfPage,
  renderTextWithReferences,
}: AS13ExamplesCustomContentProps) {
    ...
    <div className="w-full space-y-6 animate-fade-in font-sans">
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`text-xs font-bold px-3 py-1.5 rounded-t-lg transition-all whitespace-nowrap cursor-pointer flex items-center gap-1.5 border-t border-x ${
              activeTab === tab.id
                ? 'bg-slate-50 border-slate-250 border-b-transparent text-blue-600 dark:bg-slate-900/40 dark:border-gray-800 dark:border-b-transparent dark:text-blue-400'
      ...
      <div className="bg-slate-50 dark:bg-[#181E30] border border-slate-200 dark:border-slate-800 rounded-xl p-6 sm:p-8 space-y-4">
        <h4 className="font-sans font-bold text-xs uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
          <Info size={14} />
          <span>AS 13 Accounting for Investments Summary</span>
        </h4>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-[13px] leading-relaxed">
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-blue-600 dark:text-blue-400 block mb-2">Current Investments:</span>
            <p className="text-slate-650 dark:text-gray-400 text-[12px] leading-relaxed">
              Carried at the **lower of cost and fair value**. Valuation done on individual basis (Tata shares, Reliance shares separate). Reversals allowed up to cost.
            </p>
          </div>
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-amber-600 dark:text-amber-400 block mb-2">Long-term Investments:</span>
            <p className="text-slate-650 dark:text-gray-400 text-[12px] leading-relaxed">
              Carried at **cost**. Written down only if there is a decline in value that is **other than temporary**. Temporary market fluctuations are ignored.
            </p>
          </div>
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-emerald-600 dark:text-emerald-400 block mb-2">Pre-Acquisition Interest:</span>
            <p className="text-slate-650 dark:text-gray-400 text-[12px] leading-relaxed">
              Any dividend or interest earned prior to purchase date is **deducted from cost** of the investment, NOT recognized as dividend/interest income.
            </p>
          </div>
        </div>
      </div>
