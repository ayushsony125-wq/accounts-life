...
import {
  ...
  judicialCases,
  examCorner,
} from './AS16ExamplesData'

interface AS16ExamplesCustomContentProps {
...
function CaseStudyCard({
  ...
  const themeMap = {
    ...
    violet: {
      ...
      panelHeader: 'bg-violet-50/40 dark:bg-[#1A2542]',
...
export function AS16ExamplesCustomContent({
  navigateToPdfPage,
  renderTextWithReferences,
}: AS16ExamplesCustomContentProps) {
    ...
    <div className="w-full font-sans space-y-6">
      ...
      <div className="p-6 bg-gradient-to-r from-[#1E2640] to-[#111726] rounded-2xl text-white shadow-xs">
        <h2 className="text-xl sm:text-2xl font-bold mb-2 flex items-center gap-2">
          ...
          AS 16 Master Case Studies &amp; Examples
        </h2>
        <p className="text-[13px] sm:text-[14px] text-slate-350 leading-relaxed max-w-3xl">
          Learn borrowing costs capitalization under AS 16 using specific loan revaluations, capitalization rates, foreign currency loan offsets, environmental delays, and exam shortcuts.
        </p>
      </div>
