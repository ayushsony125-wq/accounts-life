...
import {
  ...
  judicialCases,
  examCorner,
} from './AS25ExamplesData'

interface AS25ExamplesCustomContentProps {
...
function CaseStudyCard({
  ...
  const themeMap = {
    ...
    slate: {
      accent: 'from-slate-500 via-slate-650 to-slate-700',
  ...
  const focusColors = {
    ...
    concept: 'bg-emerald-50 text-emerald-950 border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-200 dark:border-emerald-900/40',
...
export function AS25ExamplesCustomContent({
  navigateToPdfPage,
  renderTextWithReferences,
}: AS25ExamplesCustomContentProps) {
    ...
    <div className="w-full font-sans space-y-6">
      ...
      <div className="p-6 bg-gradient-to-r from-[#1E2640] to-[#111726] rounded-2xl text-white shadow-xs">
        <h2 className="text-xl sm:text-2xl font-bold mb-2 flex items-center gap-2">
          ...
          AS 25 Master Case Studies &amp; Examples
        </h2>
        <p className="text-[13px] sm:text-[14px] text-slate-350 leading-relaxed max-w-3xl">
          Learn Interim Financial Reporting under AS 25. Explore weighted average annual tax rate estimations, seasonal revenue recognition rules, and prior quarter restatements for policy changes.
        </p>
      </div>
