...
import {
  ...
  judicialCases,
  examCorner,
} from './AS28ExamplesData'

interface AS28ExamplesCustomContentProps {
...
function CaseStudyCard({
  ...
  const themeMap = {
    ...
    rose: {
      ...
      badge: 'bg-rose-50 text-rose-750 border-rose-200/50 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-800/30',
...
export function AS28ExamplesCustomContent({
  navigateToPdfPage,
  renderTextWithReferences,
}: AS28ExamplesCustomContentProps) {
    ...
    <div className="w-full font-sans space-y-6">
      ...
      <div className="p-6 bg-gradient-to-r from-[#1E2640] to-[#111726] rounded-2xl text-white shadow-xs">
        <h2 className="text-xl sm:text-2xl font-bold mb-2 flex items-center gap-2">
          ...
          AS 28 Master Case Studies &amp; Examples
        </h2>
        <p className="text-[13px] sm:text-[14px] text-slate-350 leading-relaxed max-w-3xl">
          Learn Impairment of Assets under AS 28. Explore recoverable amount, value in use discounting, Cash Generating Units (CGUs), bottom-up &amp; top-down tests, and impairment loss reversals.
        </p>
      </div>
