...
import {
  ...
  judicialCases,
  examCorner,
} from './AS5ExamplesData'

interface AS5ExamplesCustomContentProps {
...
function CaseStudyCard({
  ...
  return (
    <div className={`w-full bg-white dark:bg-[#111726] border border-slate-200 dark:border-gray-800 rounded-xl p-5 sm:p-7 shadow-xs border-l-4 ${c.leftBorder} flex flex-col gap-5`}>
      ...
      <div className="flex-1 text-[14px] sm:text-[14.5px] text-slate-700 dark:text-gray-300 leading-relaxed font-normal min-h-[120px] transition-all duration-150">
...
export function AS5ExamplesCustomContent({
  navigateToPdfPage,
  renderTextWithReferences,
}: AS5ExamplesCustomContentProps) {
    ...
    <div className="w-full space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-slate-50 dark:bg-[#181E30] border border-slate-200 dark:border-slate-800 rounded-xl p-6 sm:p-8 space-y-4">
        <h4 className="font-sans font-bold text-xs uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
          <Info size={14} />
          <span>AS 5 Categories Quick-Reference Guide</span>
        </h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px] leading-relaxed">
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-blue-600 dark:text-blue-400 block mb-2">Ordinary vs. Extraordinary Items:</span>
            <ul className="list-disc pl-5 space-y-1 text-slate-650 dark:text-gray-400">
              <li><strong>Ordinary Activities:</strong> Incidental activities undertaken in furtherance of business. Large/unusual ordinary items are **Exceptional Items** (disclosed separately in P&amp;L).</li>
              <li><strong>Extraordinary Items:</strong> Distinct from ordinary activities, not expected to recur regularly (e.g., natural disaster, asset attachment).</li>
            </ul>
          </div>
          <div className="p-4 bg-white dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800">
            <span className="font-bold text-amber-600 dark:text-amber-400 block mb-2">Prior Period Errors vs. Estimate Changes:</span>
            <ul className="list-disc pl-5 space-y-1 text-slate-650 dark:text-gray-400">
              <li><strong>Prior Period Items:</strong> Result from mathematical errors, omissions, or misinterpretation of facts in past years. Disclosed separately in current P&amp;L.</li>
              <li><strong>Estimate Changes:</strong> Prospective adjustments based on new developments (e.g., re-evaluating useful lives or bad debt provisions). No P&amp;L reclassification.</li>
            </ul>
          </div>
        </div>
      </div>
