...
import {
  ...
  examCorner,
  auditNotes,
} from './AS9ExamplesData'

interface AS9ExamplesCustomContentProps {
        ...
        <div className="flex items-center gap-2 shrink-0 pt-0.5">
          {pdfPage && navigateToPdfPage && (
            <button
              ...
              title={`Open ICAI AS 9 PDF — Page ${pdfPage}`}
            >
              <FileText size={11} />
            </button>
          )}
...
export function AS9ExamplesCustomContent({ navigateToPdfPage, renderTextWithReferences }: AS9ExamplesCustomContentProps) {
  ...
  const handleChapterClick = (id: string) => {
    ...
    const stickyToc = document.getElementById('as9-examples-sticky-toc')
    if (container && target) {
  ...
  const buildIllusPanels = (illus: typeof icaiIllustrations[0]): CardPanel[] => [
    ...
    {
      title: 'Conclusion & AS 9 Treatment',
  ...
  return (
    ...
    >
      ...
      <div id="as9-examples-sticky-toc" className="sticky top-[58px] bg-white dark:bg-[#111726] border-b border-slate-200 dark:border-slate-800 z-20 w-full select-none shadow-sm">
      ...
      <div className="mx-auto w-[98%] max-w-[1720px] bg-white dark:bg-[#111726] shadow-sm border border-slate-200/70 dark:border-slate-800 rounded-xl px-4 sm:px-8 lg:px-12 py-10 sm:py-14 space-y-16 relative my-4">
        ...
        <section id="sec2-icai-illustrations" className="scroll-mt-36 space-y-8 w-full">
          {renderSectionHeader('1', 'ICAI Illustrations',
            'Official ICAI AS 9 illustrations (Illustration 9.1–9.8) covering delayed delivery, installation, approval sales, consignment, installation fees, advertising, tuition, and resource use.')}
        ...
        <section id="sec2-business-cases" className="scroll-mt-36 space-y-8 w-full border-t border-slate-200 dark:border-slate-800 pt-8">
          {renderSectionHeader('2', 'Business Case Studies',
            'Realistic commercial scenarios applying AS 9 across retail, manufacturing, price protection, and bill-and-hold.')}
          <div className="space-y-12">
        ...
        <section id="sec2-audit-cases" className="scroll-mt-36 space-y-8 w-full border-t border-slate-200 dark:border-slate-800 pt-8">
          {renderSectionHeader('3', 'Audit Case Studies',
            'Statutory audit scenarios covering cut-off testing, FOB terms, and country-level transfer risk postponement.')}
          <div className="space-y-12">
            {auditCases.map((ac) => (
              <CaseStudyCard
                key={ac.id}
        ...
        <section id="sec2-regulatory-obs" className="scroll-mt-36 space-y-8 w-full border-t border-slate-200 dark:border-slate-800 pt-8">
          {renderSectionHeader('4', 'Regulatory Observations',
            'NFRA and SEBI enforcement actions related to premature revenue recognition and maintenance contracts.')}
          <div className="space-y-12">
        ...
        <section id="sec2-legal-cases" className="scroll-mt-36 space-y-8 w-full border-t border-slate-200 dark:border-slate-800 pt-8">
          {renderSectionHeader('5', 'Landmark Judicial Cases',
            'Supreme Court rulings on real income principles, managing agency commissions, and revenue accrual.')}
          <div className="space-y-12">
        ...
        <section id="sec2-exam-oriented" className="scroll-mt-36 space-y-8 w-full border-t border-slate-200 dark:border-slate-800 pt-8">
          {renderSectionHeader('6', 'Exam-Oriented Corner',
            'High-yield past exam questions, detailed answers, marks allocation, and paragraph references for AS 9.')}
            ...
            {examCorner.map((q) => (
              <div key={q.id} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 space-y-4">
                ...
                <div className="bg-white dark:bg-slate-950 p-5 rounded-xl border border-slate-250 dark:border-slate-850 space-y-2">
                  ...
                  <div className="text-[13.5px] leading-relaxed text-slate-700 dark:text-slate-350 font-serif whitespace-pre-line">
                    {q.answer}
                  </div>
                </div>
              </div>
        ...
        <section id="sec2-audit-notes" className="scroll-mt-36 space-y-8 w-full border-t border-slate-200 dark:border-slate-800 pt-8">
          {renderSectionHeader('7', 'Audit Notes & Reporting',
            'Practical checklists and testing guidance for auditors validating revenue recognition compliance.')}
