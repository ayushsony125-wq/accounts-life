...
function mapStaticEntryToStandard(staticEntry: any, framework: 'AS' | 'Ind AS'): Standard {
  ...
  return {
    ...
    pdfPagesCount: staticEntry.entrySlug === 'as-1' ? 16 : (staticEntry.entrySlug === 'as-2' ? 16 : (staticEntry.entrySlug === 'as-9' ? 12 : (staticEntry.entrySlug === 'as-10' ? 28 : 3))),
...
export async function fetchStandardDetail(id: string, framework: 'AS' | 'Ind AS'): Promise<Standard | null> {
  ...
  if (!entry) {
    ...
    if (id === 'as-10') {
      const { AS_10_ENTRY } = require('./data/static-entries')
      return mapStaticEntryToStandard(AS_10_ENTRY, 'AS')
    }
    if (id === 'ind-as-1') {
